...
import sidebarItems from "../assets/data/adminSidebar.json";
...
function AdminSidebar({ sidebarWidth, setSidebarWidth, toggleSidebar }) {
  ...
  return (
    <nav
      className={clsx(
        "_admin-sidebar sticky top-0 min-h-dvh h-fit bg-white min-w-[72px] border-e-[1px] duration-300 md:w-[250px] md:absolute md:z-50 md:h-full overflow-auto flex justify-start flex-col",
    ...
    >
      ...
      >
        ...
        >
          <Link
            className={clsx(
              "flex justify-start items-center w-full py-3 gap-3 mr-auto px-2"
            )}
            to="/"
          >
            <img src="/images/emblem.png" alt="emblem " className="w-6 h-6" />
            LPBMIS
          </Link>
          <Button
            variant="text"
          ...
          >
            <span className="material-symbols-outlined py-0  cursor-pointer text-[#1C1B1F]">
              close
            </span>
          </Button>
        </div>
