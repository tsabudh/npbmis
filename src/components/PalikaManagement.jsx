...
import ModalEditPalikaNameAddress from "./modals/ModalEditPalikaNameAddress";
import ModalEditPalikaWards from "./modals/ModalEditPalikaWards";
import ModalEditDepartments from "./modals/ModalEditDepartments";

function PalikaManagement() {
  const palika = useRecoilValue(palikaState);

  const handlePalikaNameAddressEdit = () => {
    NiceModal.show(ModalEditPalikaNameAddress);
  };

  const handlePalikaWardsEdit = () => {
    NiceModal.show(ModalEditPalikaWards, {
      initialWards: palika?.wards,
    });
  };

  const handleDepartmentsEdit = () => {
    NiceModal.show(ModalEditDepartments);
  };
  return (
          ...
          <section className="flex flex-col items-start justify-start gap-3 border border-blue-fair-300 bg-blue-fair-50  rounded-md  py-2">
            <header className="px-2 flex items-center justify-between w-full">
              ...
              <Button
                variant="filled"
                color="green-fair"
                onClick={handlePalikaNameAddressEdit}
              >
...
export default PalikaManagement;
